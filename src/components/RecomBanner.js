...
const RecomBanner = (props) => {

    const {navigation, pageRecom} = props;

    const [loading, setLoading] = useState(false);

    //console.log(props);

    const [recommendWorkData, setRecommendWorkData] = useState([]);

    const recommendWork = async () => {
        Api.send('work_recommend', [], (args)=>{ 
            let resultItem = args.resultItem;
            let arrItems = args.arrItems;
            if (resultItem.result === 'Y' && arrItems) {
               //console.log('하하', arrItems);
             
               setRecommendWorkData(arrItems);
            }
    ...
    //console.log(recommendWorkData.length);
